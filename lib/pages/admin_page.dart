...
class _AdminScreenState extends State<AdminScreen> {
  ...
  Future<void> _fetchVendors() async {
    ...
    String? token = await FlutterSecureStorage().read(key: 'authToken');
    final response = await NetworkUtil.tryRequest(
      '/api/v1/admin/getVendors',  // Assuming this is your endpoint for fetching vendors
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer $token',
      },
    ...
    return Scaffold(
      ...
      body: _isLoading
        ...
        itemBuilder: (context, index) {
          ...
          bool isApproved = _approvedRequests[request['_id']] ?? false;
          return Card(
            child: ListTile(
              title: Text('@${request['username']}'),
              subtitle: Text('${request['firstName']} ${request['lastName']} wants to become a vendor.', style: TextStyle(fontSize: 11),),
    ...
    return Scaffold(
      ...
      body: _isLoading
        ...
        child: Column(
          ...
          children: <Widget>[
            Text((_profile['firstName'] ?? '') + ' ' + (_profile['lastName'] ?? ''), style: TextStyle(fontSize: 23, fontWeight: FontWeight.bold)),
            SizedBox(height: 10),
            Text('@${_profile['username'] ?? 'N/A'}', style: TextStyle(fontSize: 15, color: Colors.grey)),
            Text('${_profile['email'] ?? 'N/A'}', style: TextStyle(fontSize: 15, color: Colors.grey)),
            SizedBox(height: 10),
